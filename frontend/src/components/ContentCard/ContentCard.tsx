...
export const ContentCard = ({
  ...
  const truncateSummary = (text: string, limit = SUMMARY_LIMIT) => {
  ...
  const isSummaryTruncated = item.summary.length > SUMMARY_LIMIT;
  ...
  return (
    <Wrapper>
      ...
      >
        ...
        >
          {isSummaryExpanded || !isSummaryTruncated
            ? item.summary
          ...
          {isSummaryTruncated ? (
            <>
              ...
              <button
                type="button"
                className={`inline-flex items-center text-[11px] font-semibold ml-1 rounded-full px-2 py-0.5 border transition-colors ${
                  isDarkMode
          ...
          >
            ...
            {item.source &&
              (disableSourceChip || linkHref || hideSourceLabel) && (
                <span
                  className={isDarkMode ? "text-gray-200" : "text-gray-400"}
                >
                  {item.source}
                </span>
              )}
